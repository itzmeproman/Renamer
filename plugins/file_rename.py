...
async def auto_rename_files(client, message):
    ...
    # Check if the user has a queue, create one if not
    if user_id not in user_queues:
        user_queues[user_id] = Queue()

    # Enqueue the incoming message to the user's queue
    user_queues[user_id].put(message)

    # Check if the queue size is greater than 5
    ...
    while not user_queues[user_id].empty():
        ...
        await auto_rename_files(client, queued_message)
